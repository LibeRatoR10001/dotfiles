...
local writeHeadPositionChange = function()
	...
	if file_object == nil then
		...
		if initPass or pitch ~= last_pitch or maximumTimeoutReached then
			changedValues[#changedValues + 1] = string.format(
				", [expr] v360 pitch 'lerp(%.3f,%.3f,(T-%.3f)/%.3f)'",
				last_pitch,
				pitch,
				lasttimePos,
				movementDuration
		...
		if initPass or yaw ~= last_yaw or maximumTimeoutReached then
			changedValues[#changedValues + 1] =
				string.format(", [expr] v360 yaw 'lerp(%.3f,%.3f,(T-%.3f)/%.3f)'", last_yaw, yaw, lasttimePos, movementDuration)
		end
		last_yaw = yaw

		if initPass or roll ~= last_roll or maximumTimeoutReached then
			changedValues[#changedValues + 1] = string.format(
				", [expr] v360 roll 'lerp(%.3f,%.3f,(T-%.3f)/%.3f)'",
				last_roll,
				roll,
				lasttimePos,
				movementDuration
		...
		if initPass or dfov ~= last_dfov or maximumTimeoutReached then
			changedValues[#changedValues + 1] = string.format(
				", [expr] v360 d_fov 'lerp(%.3f,%.3f,(T-%.3f)/%.3f)'",
				last_dfov,
				dfov,
				lasttimePos,
				movementDuration
...
local build_help_string = function()
	return table.concat({
		...
		binding_by_name('show_help'),
		' = show help\n',
		binding_by_name('res_up'),
		',',
		binding_by_name('res_down'),
		' = adjust quality\n',
		'Mouse Click = look around\n',
		binding_by_name('roll_left'),
		',',
		binding_by_name('roll_right'),
		' = roll head\n',
		'Mouse Wheel = zoom\n',
		binding_by_name('switch_stereo'),
		' = switch stereo Mode\n',
		binding_by_name('switch_eye'),
		' = switch eye side\n',
		binding_by_name('switch_scaler'),
		' = switch scaler\n',
		binding_by_name('switch_output_sbs'),
		' = toggle side by side output\n',
		binding_by_name('toggle_smooth'),
		' = toggle mouse smoothing\n',
		binding_by_name('new_log_session'),
		' = start/stop motion recording\n',
		binding_by_name('cycle_input'),
		',',
		binding_by_name('cycle_output'),
		' = cycle in and out projections\n',
		binding_by_name('reset_view'),
		' = center view\n',
	})
end
...
local closeCurrentLog = function()
	...
	if lasttimePos ~= nil and file_object ~= nil then
		...
		local stats = string.format(
			'# Duration: %s-%s (total %s) %s seconds',
